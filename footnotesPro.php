    ...
    public static function cmf_footnote_parse($content, $force = false)
    {
        ...
        if( !empty($footnoteSearchStringArr) && is_array($footnoteSearchStringArr) )
        {
            ...
            if( count($footnoteSearchStringArr) > $footnoteArrayChunk )
            {
                ...
                {
                    $footnoteSearchString = '/' . (($spaceSeparated) ? '(?<=\P{L}|^)(?<!(\p{N}))' : '') . '(?!(<|&lt;))(' . (!$caseSensitive ? '(?i)' : '') . implode('|', $footnoteSearchStringArrChunk) . ')(?!(>|&gt;))' . (($spaceSeparated) ? '(?=\P{L}|$)(?!(\p{N}))' : '') . '/u';
                    $content = self::cmf_dom_str_replace($content, $footnoteSearchString);
                }
            }
            else
            {
                $footnoteSearchString = '/' . (($spaceSeparated) ? '(?<=\P{L}|^)(?<!(\p{N}))' : '') . '(?!(<|&lt;))(' . (!$caseSensitive ? '(?i)' : '') . implode('|', $footnoteSearchStringArr) . ')(?!(>|&gt;))' . (($spaceSeparated) ? '(?=\P{L}|$)(?!(\p{N}))' : '') . '/u';
                $content = self::cmf_dom_str_replace($content, $footnoteSearchString);
            }
